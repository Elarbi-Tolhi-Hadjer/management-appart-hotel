...
$('#booking').submit(function (e) {
    ...
    var formData = {
        ...
        guests: $('#guests').val(),
        amount: $('#total_price').text(), // doit être mis à jour correctement
        email: $('#email').val(),
        phone_number: $('#contact_no').val()
    };

    if (!formData.room_id || !formData.user_id || !formData.phone_number || !formData.email) {
        $('.response').html('<div class="alert bg-danger">Veuillez remplir tous les champs obligatoires.</div>');
        return false;
    }
